<template>
  <v-container class="app">
    <v-app-bar class="app-bar">
      <!-- logo -->
      <v-img alt="weather logo" class="shrink logo" contain src="../assets/weather.jpg" min-width="65" width="65" />
    ...
    <v-card flat class="text-center">
      <p>
        <v-icon>mdi-map-marker</v-icon> tương mai, hai bà trưng, hà nội
      </p>
    </v-card>
    <v-card flat>
      <v-row class="text-center">
        ...
        <v-col cols="6 pa-0">
          <v-card class="body" v-if="hot == true">
            <div class="hot">
              <span class="sun"></span>
              <span class="sunx"></span>
            </div>
          </v-card>
          <v-card class="body" v-if="cloudy == true">
            <div class="cloudy">
              <span class="cloud"></span>
              <span class="cloudx"></span>
              <span class="cloudx1"></span>
              <span class="cloudx2"></span>
            </div>
          </v-card>
          <v-card class="body" v-if="rain == true">
            <div class="breezy">
              <ul>
                <li></li>
                <li></li>
          ...
          <v-card class="body" v-if="snow == true">
            <div class="stormy">
              <ul>
                <li></li>
                <li></li>
          ...
          <v-card class="body" v-if="night == true">
            <div class="night">
    ...
    <v-card v-if="hour == true">
      <v-data-table dense :headers="headers" :items="desserts" item-key="time" class="elevation-1">
        <template v-slot:top>
          <v-spacer></v-spacer>
    ...
    <v-card v-if="day == true">
      <v-data-table dense :headers="headers_day" :items="desserts_day" item-key="day" class="elevation-1">
        <template v-slot:top>
          <v-spacer></v-spacer>
...
<script>
  ...
  export default {
    ...
    watch: {
      noti() {
        console.log('tunt',this.noti)
        this.NhietDo = this.noti.nhietdo
        this.checkRain = this.noti.checkRain
        this.wind = this.noti.wind
        this.humidity = this.noti.humidity
        this.hot = false
        this.snow = false
        this.cloudy = false
        this.rain = false
        this.night = false
        if (parseInt(this.noti.checkRain) > 59) {
          this.rain = true
          return
        }
        if(parseInt(this.noti.nhietdo) > 33){
          this.hot = true
          return
        }else if(parseInt(this.noti.nhietdo) < 1){
          this.snow = true
          return
        }
        if(parseInt(this.noti.wind) > 49 && parseInt(this.noti.checkRain) > 29){
          this.rain = true
          return
        }
        if(parseInt(this.noti.wind) > 39){
          this.cloudy = true
          return
        }
        if(this.noti.type == 'night') {
          this.night = true
          return
        }
      }
...
<style scope>
  ...
  .breezy li:nth-child(6) {
    top: 17%;
    left: 25%;
  }
  .breezy li:nth-child(7) {
    top: 20%;
    left: 40%;
  }
  .breezy li:nth-child(8) {
    top: 19%;
    left: 35%;
  }
  .breezy li:nth-child(9) {
    top: 40%;
    left: 49%;
  }
  .breezy li:nth-child(10) {
    top: 20%;
    left: 27%;
  }
  .breezy li:nth-child(11) {
    top: 20%;
    left: 59%;
  }
  .breezy li:nth-child(12) {
    top: 20%;
    left: 48%;
  }
  .breezy li:nth-child(13) {
    top: 20%;
    left: 36%;
  }
  .breezy li:nth-child(14) {
    top: 20%;
    left: 66%;
  }
  .night {
